...
# --- LÓGICA DE GERAÇÃO COM BALANCEAMENTO (ESCADA) ---
def gerar_escalas_balanceadas(lista_usuarios):
    ...
    for idx, user in enumerate(usuarios_ordenados):
        ...
        # 1. Regra de Domingos (1x1) - Alternando pelo index para balancear
        ...
        # Escolhe um dia da semana para folgar baseado na posição do funcionário na lista
        ...
        for sem in range(0, 31, 7):
            ...
            # Se a semana não tem folga de domingo, coloca a folga no dia da "escada"
            if not (bloco['Status'] == 'Folga').any():
                idx_folga = bloco[bloco['Dia'] == dia_folga_escala].index.tolist()
                if idx_folga:
                    df.loc[idx_folga[0], 'Status'] = 'Folga'
            
            # Garante pelo menos uma folga na semana se a regra anterior falhar
