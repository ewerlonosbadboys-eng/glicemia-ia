...
# ---- Helpers de status (Folga / Trabalho) ----
FOLGA_ALIASES = {"F", "FOLGA", "OFF"}

def _is_folga(val) -> bool:
    """Considera folga tanto quando o status é 'Folga' quanto quando é 'F' (legado/planilha)."""
    if val is None:
        return False
    s = str(val).strip()
    if not s:
    ...
    if s in ("Folga", "F"):
        return True
    return s.upper() in FOLGA_ALIASES

def _is_work(val) -> bool:
    if val is None:
        return False
    s = str(val).strip()
    if not s:
        return False
    return (s in WORK_STATUSES) or (s == "Trabalho")
