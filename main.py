...
def gerar_escala_final(lista_usuarios):
    ...
    for idx, user in enumerate(lista_usuarios):
        ...
        # 2. Distribuição 5x2 - Proibido Folgas Juntas (Seg-Dom)
            ...
            while (df.iloc[sem:fim_sem]['Status'] == 'Folga').sum() < 2:
                ...
                for j in range(sem, fim_sem):
                    if df.loc[j, 'Status'] == 'Trabalho':
                        viz_ant = df.loc[j-1, 'Status'] == 'Folga' if j > 0 else False
                        viz_prox = df.loc[j+1, 'Status'] == 'Folga' if j < 30 else False
                        
                        if not viz_ant and not viz_prox:
...
with aba2:
    ...
    if st.session_state['historico']:
        st.write("### 📋 Histórico da Escala")
        for nome, df_p in st.session_state['historico'].items():
            with st.expander(f"Escala de {nome}"):
                st.dataframe(df_p)

with aba3:
    if st.session_state['historico']:
        f_ed = st.selectbox("Selecione funcionário:", list(st.session_state['historico'].keys()))
        ...
        with col_b:
            ...
            dia_ex = st.number_input("Adicionar no dia:", 1, 31, key="ex")
        ...
        if st.button("📥 BAIXAR EXCEL"):
            ...
            with pd.ExcelWriter(out, engine='openpyxl') as writer:
                ...
                for nome, df_f in st.session_state['historico'].items():
                    ...
                    row_idx += 2
            st.download_button("Baixar Agora", out.getvalue(), "escala_5x2_final.xlsx")
