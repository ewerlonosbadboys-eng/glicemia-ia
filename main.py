...
def gerar_escalas_balanceadas(lista_usuarios):
    ...
    for cat_nome, membros in cats.items():
        ...
        for user in membros:
            ...
            for sem in range(0, 31, 7):
                ...
                for d_idx in doms:
                    ...
                    if semana_idx % 2 == user.get('offset_dom', 0):
                        ...
                        # Folga Casada (Domingo + Segunda)
                        if user.get("Casada") and (d_idx + 1) < 31:
                            df.loc[d_idx+1, 'Status'] = 'Folga'
                            mapa_folgas_dia[d_idx+1] += 1
                            folgas_alocadas += 1

                # 2. Folgas Restantes (Garante 2 por semana sem erro nos dias 10 e 31)
                while folgas_alocadas < 2:
                    ...
                    # Prioriza dias onde menos gente da mesma categoria folga
            ...
            # Gerar horários com descanso de 11h
...
with aba4:
    if st.session_state['historico']:
        if st.button("📥 BAIXAR EXCEL COLORIDO"):
            out = io.BytesIO()
