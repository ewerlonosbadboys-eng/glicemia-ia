...
# ===== CARDS CLICÁVEIS (ULTRA NAV) =====
c1, c2, c3 = st.columns(3)

with c1:
    if st.button("👥 Colaboradores\nCadastros, perfil, senha e aprovações AX.", key="card_colab", use_container_width=True):
        st.session_state["sec_main"] = "👥 Colaboradores"
        st.session_state["subaba_colab"] = "👥 Colaboradores"
        st.rerun()

with c2:
    if st.button("🗓️ Escala\nGerar, consultar e imprimir a escala do mês.", key="card_escala", use_container_width=True):
        st.session_state["sec_main"] = "🗓️ Escala"
        st.rerun()

with c3:
    if st.button("⚙️ Gestão\nAjustes, férias, assinaturas e solicitações.", key="card_gestao", use_container_width=True):
        st.session_state["sec_main"] = "⚙️ Gestão"
        st.rerun()
