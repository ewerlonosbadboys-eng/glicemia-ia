...
def aplicar_tema_premium_etapa1():
    ...
    :root {
        ...
        --ax-card-2: linear-gradient(180deg, rgba(9,23,49,0.97), rgba(5,13,29,0.97));
        --ax-card-3: linear-gradient(180deg, rgba(12,31,66,0.98), rgba(6,16,35,0.98));
    }

    .stApp {
        background:
            radial-gradient(circle at 88% 4%, rgba(96,165,250,0.32), transparent 14%),
            radial-gradient(circle at 15% 0%, rgba(59,130,246,0.16), transparent 18%),
    ...
    section[data-testid="stSidebar"] {
        background:
            radial-gradient(circle at 20% 0%, rgba(79,140,255,0.12), transparent 20%),
            linear-gradient(180deg, #061125 0%, #081a38 55%, #092148 100%);
        border-right: 1px solid rgba(120, 160, 255, 0.16);
        box-shadow: inset -1px 0 0 rgba(255,255,255,0.04), 18px 0 42px rgba(0,0,0,0.22);
    }
    ...
    .stTimeInput input:focus {
        ...
        box-shadow: 0 0 0 1px rgba(79,140,255,0.22), 0 14px 28px rgba(0,0,0,0.18) !important;
    }
    ...
    [data-testid="stDataFrameResizable"] thead tr {
        background: linear-gradient(180deg, rgba(16,40,86,1), rgba(9,22,47,1)) !important;
    }
    ...
    [data-testid="stDataFrameResizable"] tbody tr:nth-child(even) td {
        background: rgba(7,17,36,0.96) !important;
    }

    [data-testid="stDataFrameResizable"] tr:hover td {
        background: rgba(10,27,58,0.98) !important;
        box-shadow: inset 0 1px 0 rgba(96,165,250,0.06), inset 0 -1px 0 rgba(96,165,250,0.06);
    }
    ...
    .stTabs [data-baseweb="tab-list"] {
        ...
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.025), 0 10px 22px rgba(0,0,0,0.10);
    }
    ...
    .stTabs [aria-selected="true"] {
        background: linear-gradient(180deg, rgba(35,99,235,0.40), rgba(29,78,216,0.32)) !important;
    ...
    .stExpander summary {
        background: linear-gradient(180deg, rgba(10,26,56,0.98), rgba(6,18,38,0.98));
        border-radius: 18px;
        font-weight: 700;
    }

    div[data-testid="stForm"] {
        background: linear-gradient(180deg, rgba(7,20,42,0.92), rgba(4,13,28,0.92));
    ...
    [data-testid="stVerticalBlock"] > [style*="flex-direction: column;"] > div:has(> div[data-testid="stMetric"]),
    [data-testid="stVerticalBlock"] > [style*="flex-direction: column;"] > div:has(> div[data-testid="stForm"]),
    ...
    [data-testid="stVerticalBlock"] > [style*="flex-direction: column;"] > div:has(> .stExpander) {
    ...
    /* Links e pseudo-navegação ficam com cara de botão leve */
    a, a:visited {
        color: #dbeafe !important;
    }

    /* Deixa labels mais fortes em áreas críticas */
    .stSelectbox label, .stMultiSelect label, .stTextInput label, .stDateInput label, .stNumberInput label {
...
def get_retificacao_competencia_por_chapa_dia(setor: str, ano: int, mes: int, chapa: str, dia: int) -> dict:
    """Retorna a última retificação exata da pessoa no dia da competência."""
    try:
        df = load_retificacoes_competencia(setor, int(ano), int(mes))
        if df is None or df.empty:
            return {}
        chapa = str(chapa or '').strip()
        dia = int(dia)
        df = df.copy()
        if 'chapa' not in df.columns or 'dia' not in df.columns:
            return {}
        df['chapa'] = df['chapa'].astype(str).str.strip()
        df['dia'] = pd.to_numeric(df['dia'], errors='coerce').fillna(0).astype(int)
        sel = df[(df['chapa'] == chapa) & (df['dia'] == dia)]
        if sel.empty:
            return {}
        if 'criado_em' in sel.columns:
            try:
                sel = sel.sort_values('criado_em', ascending=False)
            except Exception:
                pass
...
def render_app_like_top_nav(is_admin_area: bool, setor: str = ""):
    ...
    .app-top-nav-wrap .stButton > button{min-height:68px !important;height:68px !important;white-space:normal !important;line-height:1.18 !important;font-size:1.05rem !important;border-radius:18px !important;font-weight:800 !important;}
    </style>
    """, unsafe_allow_html=True)
    st.markdown('<div class="app-top-nav-wrap">', unsafe_allow_html=True)
    cols = st.columns(len(main_keys) + 1)
    for col, key in zip(cols[:-1], main_keys):
        label = cfg[key]["label"]
        with col:
            clicked = st.button(
                label,
    ...
    try:
        cur.execute(
            ...
            WHERE setor=? AND ano=? AND mes=?
              AND ((UPPER(movimento) LIKE 'ENTRA_DESTINO%' AND subgrupo_origem=? AND subgrupo_destino=?)
...
def build_rodizio_caixa_aplicado_pairs(hist_rows: list[dict]):
    enters = [r for r in (hist_rows or []) if str(r.get('movimento') or '').upper().startswith('ENTRA_DESTINO')]
    exits = [r for r in (hist_rows or []) if str(r.get('movimento') or '').upper().startswith('SAI_DESTINO')]
    pares = []
    usados_exit = set()
    for ent in enters:
        sai = None
        ciclo_ent = str(ent.get('ciclo_ref') or '')
        for idx, cand in enumerate(exits):
            if idx in usados_exit:
                continue
            if str(cand.get('ciclo_ref') or '') == ciclo_ent:
                sai = cand
                usados_exit.add(idx)
                break
        pares.append({
            ...
            'destino_nome': str((sai or {}).get('nome') or ''),
            'destino_chapa': str((sai or {}).get('chapa') or ''),
            'destino_entrada': str((sai or {}).get('entrada_antiga') or ''),
            'destino_nova_entrada': str((sai or {}).get('entrada_nova') or ''),
            'compatibilidade': str(ent.get('compat_status') or (sai or {}).get('compat_status') or ''),
            'observacao': str(ent.get('observacao') or (sai or {}).get('observacao') or ''),
            'criado_em': str(ent.get('criado_em') or (sai or {}).get('criado_em') or ''),
            'ciclo_ref': str(ent.get('ciclo_ref') or (sai or {}).get('ciclo_ref') or ''),
        })
    return pares
...
def _clone_colaborador_base(c: dict) -> dict:
    return {
        'Nome': str(c.get('Nome') or c.get('nome') or '').strip(),
        'Chapa': str(c.get('Chapa') or c.get('chapa') or '').strip(),
        'Subgrupo': str(c.get('Subgrupo') or c.get('subgrupo') or '').strip() or 'SEM SUBGRUPO',
        'Entrada': str(c.get('Entrada') or c.get('entrada') or '').strip() or '06:00',
        'Folga_Sab': bool(c.get('Folga_Sab', c.get('folga_sab', False))),
        'Setor': str(c.get('Setor') or c.get('setor') or '').strip(),
    }
...
def _rodizio_caixa_base_mes_anterior_congelado(setor: str, ano: int, mes: int) -> list[dict]:
    ...
    if competencia_fechada(setor, ano_ant, mes_ant):
        for ch in list(mapa.keys()):
            snap = get_colaborador_competencia_snapshot(setor, ch, ano_ant, mes_ant)
            if not snap:
                continue
            base = mapa[ch]
            base['Nome'] = str(snap.get('Nome') or base.get('Nome') or '').strip()
            base['Subgrupo'] = str(snap.get('Subgrupo') or base.get('Subgrupo') or '').strip() or 'SEM SUBGRUPO'
            base['Entrada'] = str(snap.get('Entrada') or base.get('Entrada') or '').strip() or '06:00'
            base['Folga_Sab'] = bool(snap.get('Folga_Sab', base.get('Folga_Sab', False)))
    ...
    if hist_rows:
        for row in hist_rows:
            ch = str(row.get('chapa') or '').strip()
            if not ch or ch not in mapa:
                continue
            mov = str(row.get('movimento') or '').upper()
            if mov.startswith('ENTRA_DESTINO'):
                mapa[ch]['Subgrupo'] = 'OPERADOR DE CAIXA 02'
                mapa[ch]['Entrada'] = str(row.get('entrada_nova') or mapa[ch].get('Entrada') or '').strip() or mapa[ch].get('Entrada', '06:00')
            elif mov.startswith('SAI_DESTINO'):
                mapa[ch]['Subgrupo'] = 'OPERADOR DE CAIXA 01'
                mapa[ch]['Entrada'] = str(row.get('entrada_nova') or mapa[ch].get('Entrada') or '').strip() or mapa[ch].get('Entrada', '06:00')
...
def simular_ajuste_complementar_rodizio_caixa_mes(
    ...
    qtd_alvo = int(_rodizio_caixa_qtd_total())
    qtd_atual = int(len(destino))
    faltam_total = max(0, qtd_alvo - qtd_atual)

    cont_dest = {}
    for d in destino:
        h = str(d.get('Entrada') or '').strip()
        cont_dest[h] = int(cont_dest.get(h, 0)) + 1
    ...
    def _pool_cands(horario_ref: str):
        ...
        for cand in origem:
            ...
            if not ch or ch in usados or ent_min is None or ref_min is None:
                continue
            diff = abs(int(ent_min) - int(ref_min))
            if diff > 60:
                continue
            dom_info_o = domingos_detalhe_map.get(ch, {'trab': tuple(), 'folga': tuple()})
            dest_refs = [x for x in destino if str(x.get('Entrada') or '').strip() == horario_ref]
            match_score = 0
            diff_dom = 999
            if dest_refs:
                best = None
                for d in dest_refs:
                    dch = str(d.get('Chapa') or '').strip()
                    m = _rodizio_match_domingos(dom_info_o, domingos_detalhe_map.get(dch, {'trab': tuple(), 'folga': tuple()}))
                    dd = abs(int(domingos_map.get(ch, 0) or 0) - int(domingos_map.get(dch, 0) or 0))
                    cand_best = (-int(m.get('trab_iguais_qtd', 0) or 0), -int(m.get('folga_iguais_qtd', 0) or 0), dd)
    ...
    for cota_info in _rodizio_caixa_cotas_custom(setor, subgrupo_destino):
        ...
        while falta_h > 0 and faltam_rest > 0:
            pool = _pool_cands(horario_ref)
            if not pool:
                alertas.append(f'Não há candidato para completar o horário {horario_ref} no {subgrupo_destino} dentro da tolerância de até 1 hora.')
                break
            cand = pool[0]
            ch = str(cand.get('Chapa') or '').strip()
            ent = str(cand.get('Entrada') or '').strip()
            usados.add(ch)
            diff = abs(int((_hora_to_min(ent) or 0)) - int((_hora_to_min(horario_ref) or 0)))
            slots.append({
                'slot_key': f'complemento|{horario_ref}|{len(slots)}|{ch}',
                ...
                'destino_chapa': '',
                'destino_entrada': '',
                'destino_nova_entrada': '',
                'destino_domingos_label': '-',
                'compatibilidade': _classificar_compat_horario(ent, horario_ref, tolerancia_min=60),
                'observacao': f'Ajuste complementar após retificação. Completa vaga do horário {horario_ref} usando base congelada da competência anterior + retificações do mês atual.',
                'horario_ref': horario_ref,
                'alternativas_mesmo_horario': max(0, sum(1 for c in origem if str(c.get('Entrada') or '').strip() == horario_ref) - 1),
                'alternativas_ate_1h': max(0, len(_pool_cands(horario_ref)) - 1),
                'fallback_horario_proximo': bool(ent != horario_ref),
                'fallback_destino_horario_proximo': False,
                'diff_horario_ref_min': int(diff),
    ...
    return {
        'qtd_destino_atual': int(qtd_atual),
        'qtd_destino_obrigatoria': int(qtd_alvo),
        'faltam': int(max(0, qtd_alvo - qtd_atual)),
        'slots': slots,
        'alertas': alertas,
    }
...
def aplicar_ajuste_complementar_rodizio_caixa_mes(setor: str, ano: int, mes: int, slots: list[dict], subgrupo_origem: str = 'OPERADOR DE CAIXA 01', subgrupo_destino: str = 'OPERADOR DE CAIXA 02'):
    slots = list(slots or [])
    if not slots:
        return {'ok': False, 'msg': 'Nenhuma sugestão complementar para aplicar.'}

    con = db_conn()
    cur = con.cursor()
    ciclo = _mes_ref_str(ano, mes) + '|ajuste'
    ts = datetime.now().isoformat()
    try:
        cur.execute('BEGIN')
        for s in slots:
            ...
            cur.execute(
                "UPDATE colaboradores SET subgrupo=?, entrada=? WHERE setor=? AND chapa=?",
                (subgrupo_destino, ent_nova, setor, ch)
            )
            cur.execute(
                """
                INSERT INTO subgrupo_competencia(setor, ano, mes, chapa, subgrupo, atualizado_em)
                ...
                ON CONFLICT(setor, ano, mes, chapa) DO UPDATE SET
                    ...
                    atualizado_em=excluded.atualizado_em
                """,
                (setor, int(ano), int(mes), ch, subgrupo_destino, ts)
            )
            cur.execute(
                """
                INSERT OR REPLACE INTO rodizio_caixa_hist(setor, ano, mes, ciclo_ref, chapa, nome, movimento, subgrupo_origem, subgrupo_destino, entrada_antiga, entrada_nova, compat_status, observacao, criado_em)
    ...
    return {'ok': True, 'msg': f'Ajuste complementar aplicado com {len(slots)} entrada(s) no {subgrupo_destino}.'}
...
def transferir_todos_caixa02_para_caixa01(setor: str, ano: int, mes: int, subgrupo_origem: str = 'OPERADOR DE CAIXA 01', subgrupo_destino: str = 'OPERADOR DE CAIXA 02'):
    ...
    try:
        ...
        fontes = [
            ...
            ("SELECT TRIM(chapa) FROM subgrupo_competencia WHERE UPPER(TRIM(setor))=UPPER(TRIM(?)) AND ano=? AND mes=? AND UPPER(TRIM(subgrupo))=UPPER(TRIM(?))", (setor, ano, mes, subgrupo_destino)),
        ...
        for chapa in sorted(chapas):
            ...
            cur.execute("UPDATE colaboradores SET subgrupo=?, entrada=COALESCE(NULLIF(?, ''), entrada) WHERE UPPER(TRIM(setor))=UPPER(TRIM(?)) AND TRIM(chapa)=?", (subgrupo_origem, entrada, setor, chapa))
            cur.execute("INSERT INTO subgrupo_competencia(setor, ano, mes, chapa, subgrupo, atualizado_em) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(setor, ano, mes, chapa) DO UPDATE SET subgrupo=excluded.subgrupo, atualizado_em=excluded.atualizado_em", (setor, ano, mes, chapa, subgrupo_origem, ts))
            cur.execute("INSERT INTO colaborador_competencia_snapshot(setor, ano, mes, chapa, nome, subgrupo, entrada, folga_sab, atualizado_em) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(setor, ano, mes, chapa) DO UPDATE SET nome=excluded.nome, subgrupo=excluded.subgrupo, entrada=excluded.entrada, folga_sab=excluded.folga_sab, atualizado_em=excluded.atualizado_em", (setor, ano, mes, chapa, nome, subgrupo_origem, entrada, folga_sab, ts))
        ...
        try:
            ...
            pass
        return {'ok': False, 'total': 0, 'msg': f'Falha ao transferir todos do Caixa 02 para Caixa 01: {e}'}
    finally:
        con.close()
    ...
    if sec_main == "👥 Colaboradores":
        ...
        if sec_col == "👥 Colaboradores":
            ...
            if not str(setor).strip().upper().startswith("FRENTECAIXA"):
                ...
                bcfg1, bcfg2, bcfg3, _bcfg4 = st.columns([1, 1, 1.5, 2.5])
                ...
                if bcfg2.button(label_reset, key='rod_caixa_reset_cfg', use_container_width=True, disabled=(_status_comp_rod == 'FECHADA')):
                    if rodizio_ja_aplicado_mes:
                        ...
                        st.session_state['_rod_caixa_reset_defaults_pending'] = True
                        st.session_state[force_review_key] = True
                        st.success('Configuração resetada para o padrão.')
                        st.rerun()
                if bcfg3.button('Transferir Caixa 02 → Caixa 01', key='rod_caixa_transfer_all', use_container_width=True, disabled=(_status_comp_rod == 'FECHADA')):
                    try:
                        res_transfer = transferir_todos_caixa02_para_caixa01(setor, ano_r, mes_r, subgrupo_origem, subgrupo_destino)
                        base_reset = f"rod_caixa_aprov::{setor}::{ano_r}::{mes_r}::{subgrupo_origem}::{subgrupo_destino}"
                        for suf in ["::aprovados", "::negados", "::aplicado", "::complementar_aprovados"]:
                            st.session_state.pop(base_reset + suf, None)
                        st.session_state[force_review_key] = True
                        if res_transfer.get('ok'):
                            st.success(res_transfer.get('msg', 'Transferência concluída.'))
                            st.rerun()
                        else:
                            st.warning(res_transfer.get('msg', 'Não foi possível transferir todos do Caixa 02 para o Caixa 01.'))
                    except Exception as e:
                        st.error(f'Falha ao transferir todos do Caixa 02: {e}')
                ...
                if rodizio_ja_aplicado_mes:
                    ...
                    comp_aprov_key = state_base + "::complementar_aprovados"
                    if comp_aprov_key not in st.session_state:
                        st.session_state[comp_aprov_key] = {}

                    if int(comp_sim.get('qtd_destino_atual', 0) or 0) < int(qtd_obrigatoria):
                        faltam = int(comp_sim.get('faltam', 0) or 0)
                        st.warning(f'Após retificação/subgrupo atual do mês, o {subgrupo_destino} ficou com {int(comp_sim.get("qtd_destino_atual", 0) or 0)} pessoa(s). A regra fixa exige {qtd_obrigatoria}. O sistema desceu {len(comp_sim.get("slots") or [])} nova(s) sugestão(ões) para completar {faltam} vaga(s).')
                        for alerta in (comp_sim.get('alertas') or []):
                            st.warning(alerta)
                        slots = list(comp_sim.get('slots') or [])
                        ...
                        if slots and int(aprovados_validos) >= int(len(slots)):
                            if st.button('➕ Aplicar ajuste complementar agora', key='rod_caixa_apply_complementar', use_container_width=True, disabled=(_status_comp_rod == 'FECHADA')):
                                ...
                                if res_comp.get('ok'):
                                    ...
                                    st.error(res_comp.get('msg', 'Não foi possível aplicar o ajuste complementar.'))
                    else:
    ...
    # overrides do mês (para respeitar folgas/férias)
        ...
        if sec_aj == "✏️ Retificar folga, horário e subgrupo":
            ...
            if not colaboradores_ret:
                ...
                idx_status = status_opts.index(valor_status) if valor_status in status_opts else 0
                novo_status = colra.selectbox("Novo status", options=status_opts, index=idx_status, key=f"ret_status_live::{setor}::{ano}::{mes}::{chapa_ret}::{dia_ret}")
                nova_entrada = colrb.text_input("Nova entrada", value=valor_ent, key=f"ret_ent_live::{setor}::{ano}::{mes}::{chapa_ret}::{dia_ret}")
                nova_saida = colrc.text_input("Nova saída", value=valor_sai, key=f"ret_sai_live::{setor}::{ano}::{mes}::{chapa_ret}::{dia_ret}")
                sub_opts = sorted({str(c.get('Subgrupo') or '').strip() for c in colaboradores_ret if str(c.get('Subgrupo') or '').strip()})
                for _sg in ['OPERADOR DE CAIXA 01', 'OPERADOR DE CAIXA 02', 'FISCAL DE CAIXA']:
                    if _sg and _sg not in sub_opts:
                        sub_opts.append(_sg)
                sub_opts = [''] + sorted({x for x in sub_opts if x})
                idx_sub = sub_opts.index(valor_sub) if valor_sub in sub_opts else 0
                novo_subgrupo = colrd.selectbox("Novo subgrupo (vale o mês inteiro)", options=sub_opts, index=idx_sub, key=f"ret_sub_live::{setor}::{ano}::{mes}::{chapa_ret}")
                ...
                if st.button("💾 Salvar retificação", key=f"ret_save_live::{setor}::{ano}::{mes}", use_container_width=True):
                    if not chapa_ret:
                        ...
                        payload_ret = {
                            ...
                            'nova_entrada': nova_entrada,
                            'nova_saida': nova_saida,
                            'novo_subgrupo': novo_subgrupo or valor_sub or base_sub,
                            'motivo_ret': motivo_ret,
                            'usuario': str(st.session_state.get('auth_nome') or st.session_state.get('auth_chapa') or ''),
                        }
                        if bool(auth.get('is_ax_lider', False)) and not bool(auth.get('is_admin', False)) and not bool(auth.get('is_lider', False)):
