...
if "pdf_relatorio_bytes" not in st.session_state:
    st.session_state.pdf_relatorio_bytes = None
    ...
    if st.sidebar.button("🧾 Gerar PDF Relatório", use_container_width=True):
        try:
            ...
            if not pdf_bytes:
                ...
                st.session_state.pdf_relatorio_nome = f"Relatorio_Saude_Kids_{agora_br().strftime('%Y-%m-%d_%H-%M')}.pdf"
                st.sidebar.success("PDF gerado com sucesso. Agora clique em baixar.")
        except Exception as e:
            st.session_state.pdf_relatorio_bytes = None
            st.session_state.pdf_relatorio_nome = ""
            st.sidebar.error(f"Erro ao gerar PDF: {e}")

    if st.session_state.pdf_relatorio_bytes:
        st.sidebar.download_button(
            "⬇️ Baixar PDF",
            data=st.session_state.pdf_relatorio_bytes,
            file_name=st.session_state.pdf_relatorio_nome or "Relatorio_Saude_Kids.pdf",
            mime="application/pdf",
            use_container_width=True,
            key="download_pdf_relatorio_sidebar",
        )
