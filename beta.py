...
ARQ_F = "feedbacks_BETA.csv" # Arquivo para mensagens
...
def init_db():
    ...
    # Adicionando o admin conforme solicitado
...
def carregar_dados_seguro(arq):
    ...
    df = pd.read_csv(arq)
    # Admin vê tudo, usuário comum vê apenas o dele
...
# ================= INTERFACE PRINCIPAL =================
lista_abas = ["📊 Glicemia", "🍽️ Nutrição", "⚙️ Receita"]
if st.session_state.user_email == "admin":
    lista_abas.append("🛡️ Admin")

abas = st.tabs(lista_abas)

with abas[0]: # Tab Glicemia
...
# ABA ADMIN (APENAS PARA USUÁRIO ADMIN)
if st.session_state.user_email == "admin":
    with abas[3]:
...
# NOVO: ÁREA DE MENSAGEM NA SIDEBAR
with st.sidebar.expander("🚀 Enviar Mensagem ao Admin"):
    txt_msg = st.text_area("Sua mensagem:", key="msg_user")
    if st.button("Enviar"):
        if txt_msg:
            agora = datetime.now(fuso_br).strftime("%d/%m/%Y %H:%M")
            novo_f = pd.DataFrame([[st.session_state.user_email, agora, txt_msg]], columns=["Usuario", "Data", "Sugestão"])
