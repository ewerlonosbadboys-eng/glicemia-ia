...
<style>
    ...
    .stTabs [data-baseweb="tab"] { background-color: #ffffff; border-radius: 10px 10px 0 0; padding: 10px 20px; }
...
def cor_glicemia_status(v):
    try:
        n = int(v)
        if n < 70: return 'background-color: #FFFFE0; color: black;' # Amarelo (Hipo)
        elif n > 180: return 'background-color: #FFB6C1; color: black;' # Vermelho (Hiper)
        elif n > 140: return 'background-color: #FFF9C4; color: black;' # Atenção
        else: return 'background-color: #C8E6C9; color: black;' # Verde (Alvo)
    except: return ''
    ...
    with tab_r:
        ...
        if st.button("Enviar Senha Temporária"):
            ...
            if conn.execute("SELECT * FROM users WHERE email=?", (r_e,)).fetchone():
                ...
                else: st.error("Falha no servidor de e-mail.")
            else: st.error("E-mail não cadastrado.")
            conn.close()
    st.stop()

# ================= LOGADO - LÓGICA E ABAS =================
...
tab1, tab2, tab3 = st.tabs(["📊 Glicemia", "🍽️ Nutrição", "⚙️ Minha Receita"])
...
with tab1:
    ...
    with c1:
        ...
        if st.button("💾 Salvar"):
            ...
            pd.concat([base, novo]).to_csv(ARQ_G, index=False)
            st.rerun()
    with c2:
        if not dfg.empty:
...
with tab2:
    ...
    if st.button("💾 Registrar Refeição"):
        ...
        pd.concat([base, novo_n]).to_csv(ARQ_N, index=False)
        st.rerun()
...
with tab3:
    ...
    if st.button("💾 Atualizar Receita"):
        ...
        pd.concat([df_r_all, nova_rec]).to_csv(ARQ_R, index=False)
        st.success("Receita salva com sucesso!")
    st.markdown('</div>', unsafe_allow_html=True)

# ================= EXPORTAÇÃO EXCEL COMPLETA =================
if st.sidebar.button("📥 Gerar Relatório Completo"):
    df_glic = carregar_dados_seguro(ARQ_G)
    df_nutri = carregar_dados_seguro(ARQ_N)
    
    if not df_glic.empty:
        output = BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            # Planilha 1: Pivot de Glicemia Colorido
            df_glic['Exibe'] = df_glic['Valor'].astype(str) + " (" + df_glic['Hora'] + ")"
            pv = df_glic.pivot_table(index='Data', columns='Momento', values='Exibe', aggfunc='last').fillna("-")
            pv.to_excel(writer, sheet_name='Glicemia')
            ...
            vermelho = PatternFill(start_color="FFB6C1", end_color="FFB6C1", fill_type="solid")
                ...
                for cell in row:
                    if cell.value and cell.value != "-":
                        try:
                            ...
                            if val < 70: cell.fill = amarelo
                            elif val > 180: cell.fill = vermelho
        ...
        st.sidebar.download_button("Clique aqui para Baixar", output.getvalue(), file_name=f"Relatorio_{st.session_state.user_email}.xlsx")
