...
def enviar_senha_nova(email_destino, senha_nova):
    # ⚠️ Segurança: NÃO deixe sua senha real no código.
    # Use senha de app do Gmail / st.secrets / variável de ambiente.
    meu_email = "ewerlon.osbadboys@gmail.com"
    minha_senha = "SUA_SENHA_DE_APP_AQUI"
        ...
        if st.button("Recuperar Acesso", use_container_width=True):
            ...
            if user:
                ...
                if enviar_senha_nova(email_alvo, nova):
                    ...
                    st.error("Erro ao enviar e-mail (verifique configuração).")
            else:
...
def _schema_receita_nova(rec: pd.Series, periodo: str) -> bool:
    # True se existir as colunas novas (faixas editáveis)
    need = [f"{periodo}_f1_min", f"{periodo}_f1_max", f"{periodo}_f1_dose",
...
def calc_insulina(v, m):
    """
    Compatível com:
    - Receita antiga: manha_f1 / manha_f2 / manha_f3 e noite_f1 / noite_f2 / noite_f3
    ...
    try:
        ...
        # ANTIGO (mantém o seu comportamento original)
        if v < 70:
            return "0 UI", "Hipoglicemia!"
        elif v <= 200:
...
if st.session_state.user_email == "admin":
    ...
    with tab1:
        ...
        with c1:
            ...
            if st.button("💾 Salvar Glicemia", use_container_width=True):
                ...
                novo = pd.DataFrame([[st.session_state.user_email, agora.strftime("%d/%m/%Y"), agora.strftime("%H:%M"),
                                      v_gl, m_gl, dose]],
    ...
    with tab2:
        ...
        st.write("### Registrar Refeição")
        m_nutri = st.selectbox("Refeição", MOMENTOS_ORDEM, key="n_m")

        # ✅ CORREÇÃO DO ERRO: multiselect precisa de opções
        ...
        if st.button("💾 Salvar Refeição", use_container_width=True):
            agora = datetime.now(fuso_br)
            novo_n = pd.DataFrame([[st.session_state.user_email, agora.strftime("%d/%m/%Y"),
                                    m_nutri, ", ".join(sel), c_tot, p_tot, g_tot]],
